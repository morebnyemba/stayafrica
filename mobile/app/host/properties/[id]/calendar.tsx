                ...
                {bookings.slice(0, 5).map((booking: any) => (
                  <View key={booking.id} className="p-3 bg-sand-50 rounded-xl">
                    <View className="flex-row justify-between items-start mb-1">
                      ...
                      <View className={`px-2 py-1 rounded-full ${booking.status === 'confirmed' ? 'bg-green-100' : 'bg-amber-100'
                        }`}>
                        <Text className={`text-xs font-semibold ${booking.status === 'confirmed' ? 'text-green-800' : 'text-amber-800'
                          }`}>
                          {booking.status}
                        </Text>
                      </View>
