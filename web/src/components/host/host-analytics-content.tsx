...
import { BarChart3, TrendingUp, Star, Calendar, DollarSign, ArrowUpRight, ArrowDownRight, Minus } from 'lucide-react';
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-primary-900">Performance Analytics</h1>
                ...
                <select
                    ...
                    className="px-4 py-2 rounded-lg border border-sand-300 bg-white text-primary-900 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500"
                >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {[
                    ...
                    return (
                        <div key={i} className="bg-white rounded-xl p-5 shadow-sm border border-sand-200/50">
                            <div className="flex items-center gap-2 mb-3">
                                <div className="w-8 h-8 rounded-lg bg-accent-100 flex items-center justify-center">
                                    <CardIcon className="w-4 h-4 text-accent-600" />
                                </div>
                                <span className="text-xs text-primary-500">{card.label}</span>
                            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-sand-200/50 overflow-hidden">
                ...
                {loading ? (
                    <div className="p-8 space-y-4">
                        {[...Array(3)].map((_, i) => (
                            <div key={i} className="animate-pulse flex gap-4">
                                <div className="h-4 w-1/4 rounded bg-sand-200" />
                                <div className="h-4 w-1/6 rounded bg-sand-200" />
                                <div className="h-4 w-1/6 rounded bg-sand-200" />
                                <div className="h-4 w-1/6 rounded bg-sand-200" />
                            </div>
                        ))}
                    ...
                    <div className="text-center py-16">
                        <BarChart3 className="w-10 h-10 mx-auto text-sand-300 mb-3" />
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-sand-200">
                                    <th className="text-left px-6 py-3 text-xs font-medium text-primary-500 uppercase tracking-wider">Property</th>
                                    <th className="text-center px-4 py-3 text-xs font-medium text-primary-500 uppercase tracking-wider">Occupancy</th>
                                    <th className="text-center px-4 py-3 text-xs font-medium text-primary-500 uppercase tracking-wider">Avg Rate</th>
                                    <th className="text-center px-4 py-3 text-xs font-medium text-primary-500 uppercase tracking-wider">Rating</th>
                                    <th className="text-center px-4 py-3 text-xs font-medium text-primary-500 uppercase tracking-wider">Bookings</th>
                                    <th className="text-right px-6 py-3 text-xs font-medium text-primary-500 uppercase tracking-wider">Revenue</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-sand-100">
                                {benchmarks.map(benchmark => (
                                    <tr key={benchmark.property_id} className="hover:bg-sand-50 transition-colors">
                                        <td className="px-6 py-4">
                                            <p className="font-medium text-primary-900 truncate max-w-[200px]">
                                                {benchmark.property_title}
                                            </p>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <span className="text-primary-900">{benchmark.occupancy_rate || 0}%</span>
                                            <div className="mt-0.5">
                                                <TrendIndicator value={benchmark.occupancy_rate || 0} benchmark={benchmark.avg_occupancy_market || 0} />
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <span className="text-primary-900">${benchmark.avg_nightly_rate || 0}</span>
                                            <div className="mt-0.5">
                                                <TrendIndicator value={benchmark.avg_nightly_rate || 0} benchmark={benchmark.avg_rate_market || 0} />
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <span className="flex items-center justify-center gap-1 text-primary-900">
                                        ...
                                        <td className="px-4 py-4 text-center text-primary-900">
                                            {benchmark.total_bookings || 0}
                                        </td>
                                        <td className="px-6 py-4 text-right font-medium text-primary-900">
