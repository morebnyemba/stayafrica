    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-primary-900">Price Breakdown</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-primary-900">Base Price</h3>
            </div>
          ...
          {pricing.applied_rules.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-semibold text-primary-900">Dynamic Pricing Adjustments</h3>
              </div>
          ...
          {pricing.fees.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-semibold text-primary-900">Fees</h3>
              </div>
              <div className="space-y-2">
                {pricing.fees.map((fee, index) => (
                  <div key={index} className="flex justify-between items-center bg-sand-50 rounded-lg p-3">
                    <span className="text-primary-700">{fee.name}</span>
                    <span className="font-medium text-primary-900">${fee.amount.toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {pricing.taxes.length > 0 && (
            <div>
              <h3 className="font-semibold text-primary-900 mb-3">Taxes</h3>
              <div className="space-y-2">
                {pricing.taxes.map((tax, index) => (
                  <div key={index} className="flex justify-between items-center bg-sand-50 rounded-lg p-3">
                    <div>
                      <span className="text-primary-700">{tax.name}</span>
                      <span className="text-sm text-primary-400 ml-2">({(tax.rate * 100).toFixed(1)}%)</span>
                    </div>
          ...
          <div className="border-t pt-4">
            <div className="flex justify-between items-center">
              <span className="text-xl font-bold text-primary-900">Total</span>
              <span className="text-3xl font-bold text-primary-600">
                ${pricing.total_price.toFixed(2)}
              </span>
            </div>
            {adjustedTotal !== baseTotal && (
              <p className="text-sm text-primary-500 mt-2 text-right">
