...
export default function PricingCalendar({ propertyId }: PricingCalendarProps) {
  ...
  const getPriceChangeIcon = (day: PricingCalendarDay) => {
    ...
    return <Minus className="w-3 h-3 text-primary-300" />;
  };
  ...
  const getPriceChangeColor = (day: PricingCalendarDay) => {
    ...
    return 'text-primary-700 bg-sand-50 border-sand-200/50';
  };
    ...
    <div className="bg-white rounded-lg shadow p-6">
              ...
              {day.is_available ? (
                <>
                  ...
                  {day.base_price !== day.dynamic_price && (
                    <p className="text-xs line-through text-primary-400">${day.base_price}</p>
                  )}
                ...
                <p className="text-sm text-primary-400">Unavailable</p>
              )}
            </div>
          );
        })}
      ...
      <div className="mt-6 flex flex-wrap gap-4 text-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-primary-500">Price increased</span>
        </div>
        <div className="flex items-center gap-2">
          <TrendingDown className="w-4 h-4 text-red-600" />
          <span className="text-primary-500">Price decreased</span>
        </div>
        <div className="flex items-center gap-2">
