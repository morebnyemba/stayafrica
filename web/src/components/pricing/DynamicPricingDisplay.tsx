  ...
  if (isLoading) {
    return (
      <div className={`animate-pulse ${className}`}>
        <div className="h-8 bg-primary-200 rounded w-32"></div>
        <div className="h-4 bg-primary-200 rounded w-24 mt-2"></div>
      </div>
    );
  }
  ...
  if (error || !pricing) {
    return (
      <div className={className}>
        ...
        <p className="text-sm text-primary-500">per night</p>
      </div>
    );
  }
      ...
      {pricing.applied_rules.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-3">
          ...
          {pricing.applied_rules.length > 2 && (
            <button
              onClick={() => setShowBreakdown(true)}
              className="px-2 py-1 text-xs font-medium text-primary-500 bg-primary-100 rounded-full hover:bg-primary-200 transition"
            >
