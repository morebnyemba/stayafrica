...
export default function POIList({ propertyId, radiusKm = 2 }: POIListProps) {
  ...
  if (!nearbyData || nearbyData.total_pois === 0) {
    return (
      <div className="bg-sand-50 rounded-lg p-12 text-center">
        <MapPin className="w-16 h-16 text-primary-400 mx-auto mb-4" />
        <h3 className="text-xl font-semibold text-primary-900 mb-2">No places found nearby</h3>
        <p className="text-primary-600">Try expanding the search radius</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h3 className="text-2xl font-bold text-primary-900">What's Nearby</h3>
        ...
        {categories.map(([category, pois]) => {
          ...
          return (
            <div key={category} className="bg-white rounded-xl shadow border">
              {/* Category Header */}
              <div className="p-4 border-b bg-sand-50 rounded-t-xl">
                <div className="flex items-center justify-between">
                  <h4 className="text-lg font-semibold text-primary-900 capitalize flex items-center gap-2">
