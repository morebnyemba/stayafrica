    ...
    <div className="bg-white rounded-lg shadow-sm border">
      <div className="p-6 border-b">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-primary-900">Message Templates</h2>
              ...
              {!previewMode ? (
                <>
                  <textarea
                    ...
                    rows={6}
                    className="w-full px-3 py-2 border border-primary-300 rounded-lg focus:ring-2 focus:ring-secondary-500 focus:border-transparent font-mono text-sm"
                    required
                  />
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-white border rounded-lg p-4 mt-3">
                    <p className="text-sm text-primary-400 mb-2">Preview:</p>
        ...
        {templates.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {templates.map((template) => (
              <div key={template.id} className="border rounded-lg p-4 hover:bg-sand-50 transition-colors">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="font-medium text-primary-900">{template.name}</h3>
                    <span className="text-xs text-primary-400">{template.template_type_display}</span>
                  </div>
                ...
                <p className="text-sm text-primary-500 line-clamp-3 mb-2">
                  {template.body}
                </p>
                ...
                {extractVariables(template.body).length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {extractVariables(template.body).map(v => (
                      <span key={v} className="px-2 py-0.5 bg-primary-100 text-primary-700 text-xs rounded">
                        {'{' + v + '}'}
                      </span>
                    ))}
                  </div>
