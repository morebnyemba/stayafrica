...
export default function IdentityVerificationManagement() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'expired':
        return 'text-primary-500 bg-sand-50';
      default:
        return 'text-primary-500 bg-sand-50';
    }
  };
  ...
  return (
    <div className="min-h-screen bg-sand-50 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-primary-900 mb-2">Identity Verification</h1>
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
            <div className="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
              <p className="text-sm text-primary-500 mb-1">Pending Review</p>
              <p className="text-2xl font-bold text-secondary-600">{stats.pending_review}</p>
            </div>
            <div className="bg-white rounded-lg shadow p-4 border-l-4 border-yellow-500">
              <p className="text-sm text-primary-500 mb-1">Total Submissions</p>
              <p className="text-2xl font-bold text-yellow-600">{stats.total_verifications}</p>
            </div>
            <div className="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
              <p className="text-sm text-primary-500 mb-1">Approved</p>
              <p className="text-2xl font-bold text-green-600">{stats.by_status.approved || 0}</p>
            </div>
            <div className="bg-white rounded-lg shadow p-4 border-l-4 border-red-500">
              <p className="text-sm text-primary-500 mb-1">Rejected</p>
              <p className="text-2xl font-bold text-red-600">{stats.by_status.rejected || 0}</p>
            </div>
            <div className="bg-white rounded-lg shadow p-4 border-l-4 border-purple-500">
              <p className="text-sm text-primary-500 mb-1">Approval Rate</p>
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-primary-100 border-b border-sand-200/50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-primary-700 uppercase tracking-wider">User</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-primary-700 uppercase tracking-wider">Document Type</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-primary-700 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-primary-700 uppercase tracking-wider">Submitted</th>
              ...
              <tbody className="divide-y divide-primary-200">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-4 text-center text-primary-400">
                      Loading...
                    </td>
                  </tr>
                  ...
                  (verifications.filter(v => !userIdFilter || v.user.id.toString() === userIdFilter)).map((verification) => (
                    <tr key={verification.id} className="hover:bg-sand-50 transition">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-primary-700 capitalize">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button
                          ...
                          className="inline-flex items-center gap-1 text-secondary-600 hover:text-secondary-700 transition"
                        >
          ...
          {totalCount > ITEMS_PER_PAGE && (
            <div className="px-6 py-4 border-t border-sand-200/50 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={page === 1}
                  className="px-4 py-2 border border-primary-300 rounded-lg text-sm font-medium text-primary-700 hover:bg-sand-50 disabled:opacity-50 disabled:cursor-not-allowed transition"
                >
                  Previous
                </button>
                <span className="px-4 py-2 text-sm text-primary-500">Page {page}</span>
                <button
